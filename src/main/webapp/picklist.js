	...
	return {
		...
		controller: function($scope, $element, $attrs, $parse){
			...
			$scope.rightShift=function(){
				 var models=modelFn($scope);
				for (var i=0; i<$scope.picklistSrc.length; i++){
					models.push($scope.itemToValue($scope.picklistSrc[i]));
				}
				$scope.removeLeftFromRight($scope.picklistSrc, $scope.srcoptions);
				$scope.picklistSrc.length=0;
			};
			...
			$scope.leftShift=function(){
				 var models=modelFn($scope);
				for (var i=0; i<$scope.picklistDest.length; i++){
					$scope.srcoptions.push($scope.picklistDest[i]);
				}
				$scope.removeLeftFromRight($scope.picklistDest, $scope.destoptions);
			...
			$scope.arrowUp=function(){
				...
				for (var i=0; i<$scope.picklistDest.length; i++){
					for (var j=0; j<$scope.destoptions.length; j++){
						if($scope.picklistDest[i]===$scope.destoptions[j]){
							idxs.push(j);
						}
					}
				}
...
'				<select multiple size=\"5\" class=\"form-control\" data-ng-options=\"fake\" name=\"fake\" data-ng-model=\"picklistSrc\" data-picklist-src>'+
'				</select>'+
'			</div>'+
...
'			</div>'+
'			<div style=\"display: table-cell; width: 40%;vertical-align: middle;\">'+
'				<select multiple size=\"5\" class=\"form-control\" data-ng-options=\"fake\" name=\"fake\" data-ng-model=\"picklistDest\" data-picklist-dest>'+
'				</select>'+
'			</div>'+
'			<div style=\"display: table-cell; width: 10%; vertical-align: middle;\" class=\"btn-group-vertical\">'+
'				<div>'+
